...
import { inferImpact, maxImpact } from "../src/conventional-commits"
...
describe("inferImpact", () => {
    describe("MAJOR impact (breaking changes)", () => {
        ...
        it("should detect breaking change in fix commit", () => {
            expect(inferImpact("fix!: breaking bug fix")).toBe("major")
        })

        it("should detect breaking change in other commit types", () => {
            expect(inferImpact("refactor!: breaking refactor")).toBe("major")
        })
    })

    describe("MINOR impact (features)", () => {
        it("should detect feat type", () => {
            expect(inferImpact("feat: add new feature")).toBe("minor")
        })

        it("should detect feat type with scope", () => {
            expect(inferImpact("feat(auth): add login functionality")).toBe("minor")
        })

        it("should detect feat type with complex scope", () => {
            expect(inferImpact("feat(api-gateway): add new endpoint")).toBe("minor")
        })

        it("should handle feat with multiline message", () => {
            const message = "feat: add authentication\n\nThis adds JWT authentication\nto the API"
            expect(inferImpact(message)).toBe("minor")
        })
    })

    describe("PATCH impact (fixes)", () => {
        it("should detect fix type", () => {
            expect(inferImpact("fix: correct typo")).toBe("patch")
        })

        it("should detect fix type with scope", () => {
            expect(inferImpact("fix(parser): handle edge case")).toBe("patch")
        })

        it("should handle fix with multiline message", () => {
            const message = "fix: resolve memory leak\n\nFixed issue with event listeners"
            expect(inferImpact(message)).toBe("patch")
        })
    })

    describe("NONE impact (other types)", () => {
        it("should return NONE for docs type", () => {
            expect(inferImpact("docs: update README")).toBe("none")
        })

        it("should return NONE for style type", () => {
            expect(inferImpact("style: format code")).toBe("none")
        })

        it("should return NONE for refactor type", () => {
            expect(inferImpact("refactor: clean up code")).toBe("none")
        })

        it("should return NONE for perf type", () => {
            expect(inferImpact("perf: optimize query")).toBe("none")
        })

        it("should return NONE for test type", () => {
            expect(inferImpact("test: add unit tests")).toBe("none")
        })

        it("should return NONE for build type", () => {
            expect(inferImpact("build: update dependencies")).toBe("none")
        })

        it("should return NONE for ci type", () => {
            expect(inferImpact("ci: configure GitHub Actions")).toBe("none")
        })

        it("should return NONE for chore type", () => {
            expect(inferImpact("chore: update config")).toBe("none")
        })

        it("should return NONE for revert type", () => {
            expect(inferImpact("revert: undo last commit")).toBe("none")
        })
    })

    describe("Invalid format", () => {
        it("should return NONE for empty string", () => {
            expect(inferImpact("")).toBe("none")
        })
        ...
        it("should return NONE for message without colon", () => {
            expect(inferImpact("feat add new feature")).toBe("none")
        })

        it("should return NONE for message without description", () => {
            expect(inferImpact("feat:")).toBe("none")
        })
        ...
        it("should return NONE for uppercase type", () => {
            expect(inferImpact("FEAT: add feature")).toBe("none")
        })

        it("should return NONE for invalid format", () => {
            expect(inferImpact("random commit message")).toBe("none")
        })

        it("should return NONE for merge commit", () => {
            expect(inferImpact("Merge branch 'main' into develop")).toBe("none")
        })
    })

    describe("Edge cases", () => {
        it("should handle type with unclosed scope parenthesis", () => {
            expect(inferImpact("feat(scope: add feature")).toBe("none")
        })

        it("should handle type with no space after colon", () => {
            expect(inferImpact("feat:add feature")).toBe("minor")
        })
        ...
        it("should handle scope with special characters", () => {
            expect(inferImpact("feat(api-v2): add feature")).toBe("minor")
        })

        it("should prioritize ! marker over type", () => {
            expect(inferImpact("chore!: breaking chore")).toBe("major")
        })
    })
})
...
describe("maxImpact", () => {
    ...
    describe("Multiple messages", () => {
        it("should return MAJOR when any message is breaking", () => {
            const impacts: VersionIncrement[] = [
                "minor",
                "patch",
                "major",
                "none"
